				...
				<div id="portfolio" class="portfolio-full-description">
				    ...
				    <div class="portfolios hentry work group">
				        <div class="work-thumbnail">
				            <a class="thumb"><img src="{{ asset('corporate')}}/images/projects/{{ $portfolio->images->max }}" alt="{{ $portfolio->title }}" title="{{ $portfolio->title }}"></a>
				        </div>
				        <div class="work-description">
				            <p>{{ str($portfolio->text)->limit(460) }}</p>
				            <div class="clear"></div>
				    ...
				    <h3>{{ __('Other Projects') }}</h3>
                    @if(!$portfolios->isEmpty())
                        <div class="portfolio-full-description-related-projects">
                            @foreach($portfolios as $portfolio)
                                <div class="related_project">
                                    <a class="related_proj related_img" href="{{ route('portfolio.show', $portfolio->id) }}" title="Love"><img src="{{ asset('corporate')}}/images/projects/{{ $portfolio->images->mini }}" alt="{{ $portfolio->title }}" title="{{ $portfolio->title }}" /></a>
